function compileMuscleData_r_v2
...
load muscle_analysis_r_results.mat

for i = 1:length(Files)
    
    muscleAnalFileName = Files(i).name;
    
    TrialName = strrep(muscleAnalFileName,'_muscle_analysis_r.xls','')
    FolderName = [TrialName '_Analyses'];
    motionFileName = strrep(muscleAnalFileName,'_muscle_analysis_r.xls','_ik.mot');
    statesFileName = strrep(muscleAnalFileName,'_muscle_analysis_r.xls','_StatesReporter_states.sto');
    IDFileName = strrep(muscleAnalFileName,'_muscle_analysis_r.xls','_id.sto');
    emgFileName = strrep(muscleAnalFileName,'_muscle_analysis_r.xls','_EMG.mot');
...
%     IDloads = IDloads(:,:);
    ...
    emgLabels = emgLabels(18:33);
    emgData = emgData(:,17:32);

    % Take desired DOFs from coordinates (left leg only, sagittal plane)
    coords = coords(1:141,[7 8 10 12 13 9]);
%     jointVels = states(1:141,[54 55 57 59 60 56]-8);
    ...
    HipMA = HipMA(:,1:35);
    HipAAMA = HipAAMA(:,1:35);
    KneeMA = KneeMA(:,1:35);
    AnkleMA = AnkleMA(:,1:35);
    SubtalarMA = SubtalarMA(:,1:35);
    
    MuscNames = muscAnalysisData(i).ColumnLabels;
    Lmt = Lmt(:,1:35);
    nMusc = length(1:35);
...
save Patient4_etmData_right_reprocessed.mat etmData
