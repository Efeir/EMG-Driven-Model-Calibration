...
for i = nTrials:-1:1
    ...
    if max(max(abs(diff(HipMA(:,:,i))))./max(abs(HipMA(:,:,i))))>.33
        %         fprintf('\nRemove because of Hip FE\n')
        Time(:,i) = [];
        JAngles(:,:,i) = [];
        JVels(:,:,i) = [];
        HipMA(:,:,i) = [];
        ...
        Vmt(:,:,i) = [];
        IDloads(:,:,i) = [];
    elseif max(max(abs(diff(HipAAMA(:,:,i))))./max(abs(HipAAMA(:,:,i))))>.33
        %         fprintf('\nRemove because of Hip AA\n')
        Time(:,i) = [];
        JAngles(:,:,i) = [];
        JVels(:,:,i) = [];
        HipMA(:,:,i) = [];
        ...
        Vmt(:,:,i) = [];
        IDloads(:,:,i) = [];
    elseif max(max(abs(diff(KneeMA(:,:,i))))./max(abs(KneeMA(:,:,i))))>.33
        %         fprintf('\nRemove because of Knee\n')
        Time(:,i) = [];
        JAngles(:,:,i) = [];
        JVels(:,:,i) = [];
        HipMA(:,:,i) = [];
        ...
        Vmt(:,:,i) = [];
        IDloads(:,:,i) = [];
    elseif max(max(abs(diff(AnkleMA(:,:,i))))./max(abs(AnkleMA(:,:,i))))>.33
        %         fprintf('\nRemove because of Ankle\n')
        Time(:,i) = [];
        JAngles(:,:,i) = [];
        JVels(:,:,i) = [];
        HipMA(:,:,i) = [];
        ...
        Vmt(:,:,i) = [];
        IDloads(:,:,i) = [];
    elseif max(max(abs(diff(SubtalarMA(:,:,i))))./max(abs(SubtalarMA(:,:,i))))>.33
        %         fprintf('\nRemove because of Subtalar\n')
        Time(:,i) = [];
        JAngles(:,:,i) = [];
        JVels(:,:,i) = [];
        HipMA(:,:,i) = [];
...
for i = 1:nMusc
    if MuscRef(i) == 1
        Mat = [onesCol HipFEMat HipAAMat HipIEMat InteractionFEAA InteractionIEAA InteractionFEIE;...
        ...
        Vec = [Lmt(:,i); Vmt(:,i);HipFEMA(:,i);HipAAMA(:,i)];
        Coefs{i} = (Mat\Vec);
    elseif MuscRef(i) == 2
        Mat = [onesCol HipFEMat HipAAMat HipIEMat KneeMat InteractionFEAA InteractionIEAA InteractionFEIE;...
        ...
        Vec = [Lmt(:,i); Vmt(:,i);HipFEMA(:,i);HipAAMA(:,i);KneeMA(:,i)];
        Coefs{i} = (Mat\Vec);
    elseif MuscRef(i) == 3
        Mat = [onesCol KneeMat;...
            0*onesCol KneeVelMat;...
            0*onesCol KneeMAMat];
        Vec = [Lmt(:,i); Vmt(:,i);KneeMA(:,i)];
        Coefs{i} = (Mat\Vec);
    elseif MuscRef(i) == 4
        Mat = [onesCol KneeMat AnkleMat SubtalarMat InteractionAnkleSub;...
        ...
        Vec = [Lmt(:,i); Vmt(:,i);KneeMA(:,i);AnkleMA(:,i);SubtalarMA(:,i)];
        Coefs{i} = (Mat\Vec);
    elseif MuscRef(i) == 5
        Mat = [onesCol AnkleMat SubtalarMat InteractionAnkleSub;...
        ...
        Vec = [Lmt(:,i); Vmt(:,i);AnkleMA(:,i);SubtalarMA(:,i)];
        Coefs{i} = (Mat\Vec);
    end
end

%% Re-extract original data
...
for i = 1:TrialsStep:nTrialsOrig
    ...
    JVels(:,:,j) = Data(i).JointVelocities(:,:);
    HipMA(:,:,j) = Data(i).HipFEMomentArms(:,:);
    HipAAMA(:,:,j) = Data(i).HipAAMomentArms(:,:);
    KneeMA(:,:,j) = Data(i).KneeMomentArms(:,:);
    AnkleMA(:,:,j) = Data(i).AnkleMomentArms(:,:);
...
save etmData_l_refitted_both.mat etmData_l etmData_r

% MuscleFits.Mats = Mat;
MuscleFits.Coefs = Coefs;
MuscleFits.MuscRef = MuscRef;

save MuscleFits_both.mat MuscleFits
